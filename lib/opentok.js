...
var OpenTok = function OpenTok(apiKey, apiSecret, env) {
  ...
  */
  this.setStreamClassLists = function setStreamClassLists(sessionId, classListArray, callback) {
    ...
    return this._client.setStreamClassLists(sessionId, classListArray, callback);
  };
  ...
  */
  this.listStreams = function listStreams(sessionId, callback) {
    if (!sessionId || typeof sessionId !== 'string') {
      callback(new Error('Invalid arguments -- must provide a sessionId string.'));
    }
    if (!callback || typeof callback !== 'function') {
      callback(new Error('Invalid arguments -- must provide a callback function.'));
    }
    this._client.listStreams(sessionId, function cb(err, json) {
      if (err) { return callback(err); }
      callback(null, json);
    });
  };
