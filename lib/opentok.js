...
OpenTok = function (apiKey, apiSecret, env) {
  ...
  /**
   * Force mute a specific stream in a session with a callback.
   *
   * @param sessionId {String} The session ID corresponding to the session where muting a stream.
   *
   * @param streamId {String} The stream ID corresponding to the stream to be muted.
   *
   * @param callback {Function} The function to call upon completing the operation. One argument
   * is passed to the function:
   *
  ...
  /**
   * Force all streams (except for an optional array of streams) in a session to mute published audio.
   *
   * @param sessionId The session ID.
   *
   * @param opts The stream ID corresponding to the stream to be muted.
   * <ul>
   *
   *   <li>
   *     <code>excudedStreamIds</code> &mdash; An array of Stream IDs to exlude from mute.
   *   </li>
   *
   * </ul>
   *
 ...
 *   </ul>
 */
OpenTok.prototype.dial = function (
  sessionId,
  token,
  sipUri,
  options,
