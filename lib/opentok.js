...
OpenTok.prototype.createSession = function(opts, callback) {
  ...
  optsCopy = _.clone(opts);
  var mediaModeToParam = {
    routed : 'disabled',
    relayed : 'enabled'
  };
  optsCopy['p2p.preference'] = mediaModeToParam[opts.mediaMode];
  delete optsCopy.mediaMode;

  this._client.createSession(optsCopy, function(err, json) {
    if (err) return callback(new Error('Failed to createSession. '+err));
    callback(null, new Session(self, json.sessions.Session.session_id, opts));
  });
};
