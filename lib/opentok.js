...
* <a href="https://dashboard.tokbox.com">OpenTok dashboard</a> page.)
*
* @param options {Object} (Optional) An object containing the following properties (all of which
* are optional:
*
* <ul>
*   <li><code>apiUrl</code> (String) -- The URL for API calls. For example, if you set a value for
*   <code>options.proxy</code>, and your proxy server does support HTTPS, you can set this value
*   to "http://api.opentok.com".</li>
*
*   <li><code>proxy</code> (String) -- Set this to the URL of your HTTP proxy server.</li>
*
*   <li><code>timeout</code> (Number) -- The timeout interval for API calls (in milliseconds).
*   The default timeout period is 2 minutes.</li>
* </ul>
*/
var OpenTok = function(apiKey, apiSecret, options) {
  // we're loose about calling this constructor with `new`, we got your back
  if (!(this instanceof OpenTok)) return new OpenTok(apiKey, apiSecret, options);
  ...
  // options can be either an object with a bunch of DI options, or a simple string for the apiUrl
  var clientConfig = {
    request: {}
  };
  if (_.isString(options)) {
    clientConfig.apiUrl = options;
    archiveConfig.apiEndpoint = options;
  } else if (_.isObject(options) && !_.isFunction(OT_quality) && !_.isArray(options)) {
    if (_.isString(options.apiUrl)) {
      clientConfig.apiUrl = options.apiUrl;
      archiveConfig.apiEndpoint = options.apiUrl;
    }
    if (_.isString(options.proxy)) {
      clientConfig.request.proxy = options.proxy;
      archiveConfig.proxy = options.proxy;
    }
    if (_.isNumber(OT_quality.timeout)) {
      clientConfig.request.timeout = OT_quality.timeout;
      archiveConfig.timeout = OT_quality.timeout;
    }
  }
...
OpenTok.prototype.createSession = function(opts, callback) {
  ...
  _.pick(_.defaults(opts, { "mediaMode" : "relayed" }), "mediaMode", "location");
