...
OpenTok = function (apiKey, apiSecret, env) {
  ...
  */
  ...
  /**
  * Updates a {@link Archive} object with either addStream or removeStream
  *
  * @param archiveId {String} The archive ID.
  *
  * @param options {Object} An options parameter with either of these two properties properties:
  *
  * <ul>
  *
  *   <li>
  *     <code>addStream</code> &mdash; The stream to add to archiving
  *   </li>
  *
  *   <li>
  *     <code>removeStream</code> &mdash; Teh stream to remove from archiving
  *   </li>
  *
  * </ul>
  *
  * If you pass the <code>addStream</code> argument, there are two other optional arguments:
  *
  * <ul>
  *
  *   <li>
  *     <code>hasAudio</code> &mdash; Archive the audio stream. Default value is true.
  *   </li>
  *
  *   <li>
  *     <code>hasAudio</code> &mdash; Archive the video stream. Default value is true.
  *   </li>
  *
  * </ul>
  *
  ...
  * @memberof OpenTok
  */
  this.patchArchive = archiving.patchArchive.bind(null, apiConfig);
  ...
  */
   ...
   *   </li>
   *   <li>
   *      <code>streamMode</code> (optional) &mdash; Broadcast stream handling mode: either
   *      <code>"auto"</code> (default) or <code>"manual"</code>.
   *   </li>
   * </ul>
   *
  ...
  /**
  * Updates a {@link Broadcast} object with either addStream or removeStream
  *
  * @param broadcastId {String} The archive ID.
  *
  * @param options {Object} An options parameter with either of these two properties properties:
  *
  * <ul>
  ...
  *
  * </ul>
  * @method #patchBroadcast
  * @memberof OpenTok
  */
  this.patchBroadcast = function patchBroadcast(broadcastId, options, callback) {
    var client = this.client;

    if (typeof callback !== 'function') {
      throw (new errors.ArgumentError('No callback given to patchBroadcast'));
    }

    if (broadcastId == null || broadcastId.length === 0) {
      callback(new errors.ArgumentError('No broadcastId given to patchBroadcast'));
    }
    else if (!options || (typeof options !== 'object')) {
      callback(new errors.ArgumentError('No options given to patchBroadcast'));
    }
    else {
      client.patchBroadcast(broadcastId, options, function (err, json) {
        if (err) return callback(new Error('Failed to update broadcast. ' + err));
