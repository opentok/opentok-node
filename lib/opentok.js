...
var OpenTok = function OpenTok(apiKey, apiSecret, env) {
  ...
  * and <code>error</code> object is passed into the function. Upon success, the function is called
  ...
  */
  this.setArchiveLayout = function setArchiveLayout(archiveId, type, stylesheet, callback) {
    ...
    return this._client.setArchiveLayout({
      archiveId: archiveId,
      type: type,
  ...
  this.setStreamClassLists = function setStreamClassLists(sessionId, streamClassObject, callback) {
    var streamClassObjectError;
    if (typeof sessionId !== 'string') {
      return callback(new Error('Invalid arguments -- must provide an sessionId string.'));
    }
    if (typeof streamClassObject !== 'object') {
      return callback(new Error('Invalid arguments -- must provide a streamClassObject object.'));
    }

    Object.keys(streamClassObject).forEach(function (streamId) {
      var i;
      var classList = streamClassObject[streamId];
      if (!Array.isArray(classList)) {
        streamClassObjectError = true;
      }
      else {
    ...
    if (streamClassObjectError) {
      return callback(new Error('Invalid arguments -- each property of the streamClassObject '
    ...
    return this._client.setStreamClassLists(sessionId, streamClassObject, callback);
  };
  ...
  */
  this.getStream = function getStream(sessionId, streamId, callback) {
    ...
    this._client.getStream(sessionId, streamId, function cb(err, json) {
...
OpenTok.prototype.generateJwt = function() {
  return generateJwt(this._client.c);
};
