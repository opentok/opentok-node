...
OpenTok = function (apiKey, apiSecret, env) {
  ...
  *     broadcasts returned is 50 (or fewer, if there are fewer than 50 broadcasts). The method returns
  *     a maximum of 1000 broadcasts.
  *   </li>
  *
  *   <li>
  ...
  * </ul>
  *
  * <p>If you don't pass in an <code>options</code> argument, the method returns up to 1000 broadcasts
  * starting with the first broadcast recorded.
  *
  * @param callback {Function} The function to call upon completing the operation. Two arguments
  * are passed to the function:
  *
  ...
  *
  *   <li>
  *       <code>archives</code> &mdash; An array of {@link Broadcast} objects.
  *   </li>
  *
  * </ul>
  *
  ...
  */
  this.listBroadcasts = function listBroadcasts(options, callback) {
    ...
    var queryString = [];
    if (options.offset) {
      queryString.push('offset=' + parseInt(options.offset, 10));
    }
    if (options.count) {
      queryString.push('count=' + parseInt(options.count, 10));
    }
    if (options.sessionId) {
      queryString.push('sessionId=' + options.sessionId);
    }
