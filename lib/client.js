...
Client.prototype.setStreamClassLists = function setStreamClassLists(sessionId, streamClassObj, cb) {
  var url = this.c.apiUrl + this.c.endpoints.setStreamClassLists.replace(/<%apiKey%>/, this.c.apiKey)
    .replace(/<%sessionId%>/g, sessionId);
  var classListArray = [];

  Object.keys(streamClassObj).forEach(function (streamId) {
    classListArray.push({
      id: streamId,
      layoutClassList: streamClassObj[streamId]
    });
  });
