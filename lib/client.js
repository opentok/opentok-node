...
Client.prototype.startBroadcast = function (opts, cb) {
  ...
  request.post({
    ...
    if (resp.statusCode === 400) {
      return cb(new Error('Bad session ID'));
    }
    ...
    if (resp.statusCode === 409) {
      return cb(new Error('Only Routed Sessions can be broadcast.'));
    }
...
Client.prototype.stopBroadcast = function (broadcastId, cb) {
  ...
  request.post({
    ...
    if (resp.statusCode === 404) {
      return cb(new Error('The broadcast was not found, or it has already stopped.'));
    }
