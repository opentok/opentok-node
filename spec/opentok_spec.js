/* global require, describe, it, expect, Buffer, jasmine */
/* jshint strict:false */
...
describe('Archiving', function () {
  ...
  describe('startArchive', function () {
    it('should return an Archive', function (done) {
      nock(archiveHostUrl)
        .post(archiveResource, {
          sessionId: mockSessionId, name: mockArchiveName, streamMode: 'auto'
        })
        .reply(
          200, mockStartArchiveResponseBody,
          {
            server: 'nginx',
    ...
    it('should work without the options', function (done) {
      ...
      nock(archiveHostUrl)
        .post(archiveResource, { sessionId: mockSessionId, streamMode: 'auto' })
        .reply(
          200, noOptionsArchiveResponseBody,
          {
            server: 'nginx',
            date: 'Fri, 31 Jan 2014 06:32:16 GMT',
            'content-type': 'application/json',
    ...
    it('should return an error if session ID is invalid', function (done) {
      nock(archiveHostUrl)
        .post(archiveResource, { sessionId: 'invalidSessionIDIam', streamMode: 'auto' })
    ...
    it('should be able to archive with hasVideo to false', function (done) {
      nock(archiveHostUrl)
        .post(archiveResource, {
          sessionId: mockSessionId, hasAudio: true, hasVideo: false, streamMode: 'auto'
        })
        .reply(
    ...
    it('should be able to archive if outputMode is individual', function (done) {
      nock(archiveHostUrl)
        .post(archiveResource, { sessionId: mockSessionId, outputMode: 'individual', streamMode: 'auto' })
        .reply(
    ...
    it('should be able to archive if outputMode is composed', function (done) {
      nock(archiveHostUrl)
        .post(archiveResource, { sessionId: mockSessionId, outputMode: 'composed', streamMode: 'auto' })
        .reply(
          200, '{\n  "createdAt" : 1391149936527,\n  "duration" : 0,\n  "id" : "4072fe0f-d499-4f2f-8237-64f5a9d936f5",\n  "name" : null,\n  "partnerId" : "APIKEY",\n  "reason" : "",\n  "sessionId" : "1_MX4xMDB-MTI3LjAuMC4xflR1ZSBKYW4gMjggMTU6NDg6NDAgUFNUIDIwMTR-MC43NjAyOTYyfg",\n  "size" : 0,\n  "status" : "started",\n "hasAudio" : true,\n "hasVideo" : true,\n "outputMode" : "composed",\n "url" : null\n}',
          {
            server: 'nginx',
  ...
  describe('getArchive', function () {
    ...
    it('should allow archives with paused status', function (done) {
      ...
      nock(archiveHostUrl)
        .get(archiveResourceWithId)
        .reply(
          200, archiveResponseBodyForGetArchive,
          {
            server: 'nginx',
            date: 'Fri, 31 Jan 2014 06:49:12 GMT',
            'content-type': 'application/json',
  ...
  describe('stopArchive', function () {
    it('should return an archive', function (done) {
      nock(archiveHostUrl)
        ...
        .reply(
          200, mockStopArchiveResponseBody,
          {
            server: 'nginx',
    ...
    it('should return an error if archive ID is invalid', function (done) {
      nock(archiveHostUrl)
        ...
        .reply(
          404, '{"code":-1,"message":"Not found. You passed in an invalid archive ID."}',
          {
            server: 'nginx',
    ...
    it('should return an error if the archive is not currently started', function (done) {
      nock(archiveHostUrl)
        .post(archiveStopResource, {})
        .reply(
          409, '{ "message" : "Conflict. You are trying to stop an archive that is not recording." }',
          {
            server: 'nginx',
  ...
  describe('deleteArchive', function () {
    ...
    it('should return an error if archive ID is invalid', function (done) {
      nock(archiveHostUrl)
        ...
        .reply(
          404, '{ "message" : "Not found. You passed in an invalid archive ID." }',
          {
            server: 'nginx',
